...
void main(void)
{
	...
	while(OPE)
	{
		switch(OPE)
		{
			...
			break;
			case 3: Nuevos_Porductos(INV, &TAM);
			break;
...
void Reabastecimiento(producto A[], int T)
{
	...
	while(CLA)
	{
		...
		while((I < T) && (A[I].clave < CLA))
		{
			I++;
			if((I==T) && (A[I].clave > CLA))
				printf("\nLa calve del producto ingresada es incorrecta");
			else
			{
				printf("\tCantidad: ");
				scanf("%d", &CAN);
...
{
	int CLA, I, J;
	printf("\nIngresa clave del producto -0 para salir-: ");
	scanf("%d", &CLA);
	while ((*T < 30 && (CLA)))
	{
		I = 0;
		while((I < *T) && (A[I].clave < CLA))
			...
			if(I == *T)
			{
				...
				*T = *T + 1;
				...
				if(A[I].clave == CLA)
					printf("\nEl producto ya se encuentra en el investario");
				else
				{
					for(J=*T;J<I;J--)
					A[J] = A[J-1];
